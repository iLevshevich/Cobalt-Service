#ifndef __MANAGER_TCP_TO_TCP_CONFIG_CHAIN_H_INCLUDED__
#define __MANAGER_TCP_TO_TCP_CONFIG_CHAIN_H_INCLUDED__

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
#	pragma once
#endif

#include "../../../fwdheaders.h"

#pragma pack( push, 8 )

namespace Cobalt
{
   namespace Core
   {
      namespace Chain
      {
         class TcpToTcpConfigChain :
            public IConfigChain {
         public:
            typedef typename config_type config_type;
            typedef typename return_type return_type;

            typedef typename Utility::Traits< boost::shared_ptr< IConfigChain > >::const_reference_t chain_type;
            typedef typename Utility::Traits< boost::shared_ptr< IConfigChain > >::base_t chain_base_type;

            typedef typename Utility::Traits< boost::shared_ptr< Factory::IImportFactory< Import::ImportType::Tcp > > >::base_t factory_base_type;
            typedef typename Utility::Traits< boost::shared_ptr< Import::Utility::ISettingsBuilder< Import::ImportType::Tcp > > >::base_t builder_base_type;
            typedef typename Utility::Traits< boost::shared_ptr< Import::Utility::ISettings< Import::ImportType::Tcp > > >::base_t settings_base_type;

            static chain_base_type getInstance() {
               return Utility::make_shared_pool_enable< IConfigChain, TcpToTcpConfigChain, boost::default_user_allocator_malloc_free >();
            }

            static chain_base_type getInstance( chain_type next ) {
               return Utility::make_shared_pool_enable< IConfigChain, TcpToTcpConfigChain, boost::default_user_allocator_malloc_free >( next );
            }

            return_type __stdcall getDispatcher( config_type config ) override {
               const Handler::Dispatcher::Type::DispatcherHandlerType type( Handler::Dispatcher::Type::StringToDispatcherHandlerType( config->second.get< std::string >( "type" ) ) );
               if( type == Handler::Dispatcher::Type::DispatcherHandlerType::TcpToTcp ) {
                  ///
                  const factory_base_type factory( getFactory< Import::ImportType::Tcp >() );
                  const settings_base_type input_settings( getSettings< Handler::Property::input >( config ) );
                  const settings_base_type output_settings( getSettings< Handler::Property::output >( config ) );

                  const std::string id( config->second.get< std::string >( "id" ) );
                  const std::string description( config->second.get< std::string >( "description" ) );

                  return Factory::ObjectFactory< std::string >::getTcpToTcpDispatcherHandler(
                     Factory::_inputFactory = factory,
                     Factory::_inputSettings = input_settings,
                     Factory::_outputFactory = factory,
                     Factory::_outputSettings = output_settings,
                     Factory::_id = id,
                     Factory::_description = description
                  );
               }
               else {
                  if( next_.get() ) {
                     return next_->getDispatcher( config );
                  }
               }

               return return_type();
            }

            virtual ~TcpToTcpConfigChain() = default;

         protected:
            TcpToTcpConfigChain() :
               next_( nullptr ) {
            }

            explicit TcpToTcpConfigChain( chain_type next ) :
               next_( next ) {
            }

         private:
            template< Handler::Property::PropertyHandlerType _En, typename std::enable_if< _En == Handler::Property::PropertyHandlerType::input >::type * = nullptr >
            settings_base_type __fastcall getSettings( config_type config ) const {
               const factory_base_type factory( getFactory< Import::ImportType::Tcp >() );
               const builder_base_type builder( factory->getSettingsBuilder() );
               ///
               builder->setHost( config->second.get< std::string >( "input.host" ) );
               builder->setPort( config->second.get< std::string >( "input.port" ) );
               ///
               return builder->build();
            }

            template< Handler::Property::PropertyHandlerType _En, typename std::enable_if< _En == Handler::Property::PropertyHandlerType::output >::type * = nullptr >
            settings_base_type __fastcall getSettings( config_type config ) const {
               const factory_base_type factory( getFactory< Import::ImportType::Tcp >() );
               const builder_base_type builder( factory->getSettingsBuilder() );
               ///
               builder->setHost( config->second.get< std::string >( "output.host" ) );
               builder->setPort( config->second.get< std::string >( "output.port" ) );
               ///
               return builder->build();
            }

            chain_base_type next_;
         };
      }
   }
}

#pragma pack( pop )

#endif //__MANAGER_TCP_TO_TCP_CONFIG_CHAIN_H_INCLUDED__
