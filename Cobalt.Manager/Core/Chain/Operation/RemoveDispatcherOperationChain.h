#ifndef __MANAGER_REMOVE_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
#define __MANAGER_REMOVE_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
...
namespace Cobalt
{
   namespace Core
   {
      namespace Chain
      {
         class RemoveDispatcherOperationChain :
         ...
         public:
            ...
            static chain_base_type getInstance( repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, RemoveDispatcherOperationChain, boost::default_user_allocator_malloc_free >( repository );
            }

            static chain_base_type getInstance( chain_type next, repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, RemoveDispatcherOperationChain, boost::default_user_allocator_malloc_free >( next, repository );
            }

            responce_type __stdcall execute( request_type request ) override {
               const std::string operation( request.get< std::string >( "operation" ) );
               if( operation == OperationTypeToString( OperationType::RemoveDispatcher ) ) {
            ...
            virtual ~RemoveDispatcherOperationChain() = default;

         protected:
            explicit RemoveDispatcherOperationChain( repository_type repository ) :
               next_( nullptr ),
               repository_( repository ) {
            }

            explicit RemoveDispatcherOperationChain( chain_type next, repository_type repository ) :
               next_( next ),
               repository_( repository ) {
            }
...
#endif //__MANAGER_REMOVE_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
