#ifndef __MANAGER_ADD_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
#define __MANAGER_ADD_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
...
namespace Cobalt
{
   namespace Core
   {
      namespace Chain
      {
         class AddDispatcherOperationChain :
         ...
         public:
            ...
            static chain_base_type getInstance( repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, AddDispatcherOperationChain, boost::default_user_allocator_malloc_free >( repository );
            }

            static chain_base_type getInstance( chain_type next, repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, AddDispatcherOperationChain, boost::default_user_allocator_malloc_free >( next, repository );
            }

            responce_type __stdcall execute( request_type request ) override {
               const std::string operation( request.get< std::string >( "operation" ) );
               if( operation == OperationTypeToString( OperationType::AddDispatcher ) ) {
                  return repository_->add( boost::lexical_cast< std::string >( request ) );
               }
               else {
                  if( next_.get() ) {
            ...
            virtual ~AddDispatcherOperationChain() = default;

         protected:
            explicit AddDispatcherOperationChain( repository_type repository ) :
               next_( nullptr ),
               repository_( repository ) {
            }

            explicit AddDispatcherOperationChain( chain_type next, repository_type repository ) :
               next_( next ),
               repository_( repository ) {
            }
...
#endif //__MANAGER_ADD_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
