#ifndef __MANAGER_RESET_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
#define __MANAGER_RESET_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
      ...
      namespace Chain
      {
         ...
         public:
            ...
            static chain_base_type getInstance( repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, ResetDispatcherOperationChain, boost::default_user_allocator_malloc_free >( repository );
            }

            static chain_base_type getInstance( chain_type next, repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, ResetDispatcherOperationChain, boost::default_user_allocator_malloc_free >( next, repository );
            }

            responce_type __stdcall execute( request_type request ) override {
               const std::string operation( request.get< std::string >( "operation" ) );
               if( operation == OperationTypeToString( OperationType::ResetDispatcher ) ) {
                  const std::string id( request.get< std::string >( "id" ) );
                  return repository_->reset( id );
               }
               else {
                  if( next_.get() ) {
            ...
            virtual ~ResetDispatcherOperationChain() = default;

         protected:
            explicit ResetDispatcherOperationChain( repository_type repository ) :
               next_( nullptr ),
               repository_( repository ) {
            }

            explicit ResetDispatcherOperationChain( chain_type next, repository_type repository ) :
               next_( next ),
               repository_( repository ) {
            }
...
#endif //__MANAGER_RESET_DISPATCHER_OPERATION_CHAIN_H_INCLUDED__
