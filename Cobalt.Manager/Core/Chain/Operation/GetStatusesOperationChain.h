#ifndef __MANAGER_GET_STATUSES_OPERATION_CHAIN_H_INCLUDED__
#define __MANAGER_GET_STATUSES_OPERATION_CHAIN_H_INCLUDED__
      ...
      namespace Chain
      {
         ...
         public:
            ...
            static chain_base_type getInstance( repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, GetStatusesOperationChain, boost::default_user_allocator_malloc_free >( repository );
            }

            static chain_base_type getInstance( chain_type next, repository_type repository ) {
               return Utility::make_shared_pool_enable< IOperationChain, GetStatusesOperationChain, boost::default_user_allocator_malloc_free >( next, repository );
            }

            responce_type __stdcall execute( request_type request ) override {
               const std::string operation( request.get< std::string >( "operation" ) );
               if( operation == OperationTypeToString( OperationType::GetStatuses ) ) {
                  return repository_->getStatuses();
               }
               else {
                  if( next_.get() ) {
            ...
            virtual ~GetStatusesOperationChain() = default;

         protected:
            explicit GetStatusesOperationChain( repository_type repository ) :
               next_( nullptr ),
               repository_( repository ) {
            }

            explicit GetStatusesOperationChain( chain_type next, repository_type repository ) :
               next_( next ),
               repository_( repository ) {
            }
...
#endif //__MANAGER_GET_STATUSES_OPERATION_CHAIN_H_INCLUDED__
